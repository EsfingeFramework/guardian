...
public class SensitivityLevelTester {
	...
	public void testSubjectLevelLowerThanOperationLevel() {
		WrappedObj<Level> authLevel = 
					AuthorizationContext.wrapAsSubjectProp(
							MacConfig.getAuthorizationLevelKey(), Level.CLASSIFIED);
	...
	public void testSubjectLevelEqualsToOperationLevel() {
		WrappedObj<Level> authLevel = 
					AuthorizationContext.wrapAsSubjectProp(
							MacConfig.getAuthorizationLevelKey(), Level.TOP_SECRET);
	...
	public void testSensitivityLevelUseWithNonDefaultAnnotationWithoutRestriction() {
		WrappedObj<Nivel> authLevel = 
				AuthorizationContext.wrapAsSubjectProp(
						MacConfig.getAuthorizationLevelKey(), Nivel.PUBLICO);
	
		GuardedMACInterface gc = AuthorizationContext.guardObject(new GuardedMACClass(), authLevel);
		
		assertEquals("This method must always execute", true, gc.executeByAllSubjectWithAnyLevelWithNonDefaultAnnotationWithoutRestriction() );
	}
